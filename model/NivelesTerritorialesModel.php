...
class NivelesTerritorialesModel
{
    private $db;
    ...
    function get ($id)
    {
        ...
        $stmt = $this->pdo->prepare('SELECT *
                                       FROM public.sys_tipo_nivel
                                      WHERE id_sys_tipo_nivel = :id');
    ...
    function getAll ()
    {
        $stmt = $this->pdo->query('SELECT * from public.sys_tipo_nivel');
        $items = [];
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $items[] = [
                'id' => $row['id_sys_tipo_nivel'],
                'descripcion' => $row['tipo_nivel']
            ];
        }
        return $items;
    }
