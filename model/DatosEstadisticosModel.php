...
class DatosEstadisticosModel
{
    ...
    function get ($id)
    {
        ...
        $stmt = $this->pdo->prepare('SELECT *
                                       FROM banco_indicadores.tbl_datos_estadisticos
                                      WHERE id = :id');
    ...
    function getAll ()
    {
        $stmt = $this->pdo->query('SELECT * from banco_indicadores.tbl_datos_estadisticos');
        $items = [];
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $items[] = [
                'id' => $row['id'],
                'descripcion' => $row['descripcion'],
                'id_nivel_territorial' => $row['id_nivel_territorial']
            ];
        }
        return $items;
    }
/*
    function insert ($rec)
    {
        try {
            ...
            return $this->pdo->lastInsertId('tbl_indicador_id_seq');
        } catch (Exception $e){
            $err = "";
